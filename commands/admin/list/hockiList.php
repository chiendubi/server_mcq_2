...
class hockiList{
    ...
    function saveFormData(){
        ...
        if($data['default']['id'] > 0){
            ...
            $data['default']['code'] = Utility::processedCheckField('c_hocki', 'code', $data['default']['name'], true);
        }
        ...
        if($name_exist == 1){
            ...
            $result = Utility::processedSaveData('c_hocki', $data['default'], $skip, $json);
            if($result['status'] == "OK"){
                $response['status'] = 'OK';
    ...
    function checkNameUsed($data){
        ...
        $is_exist = 0;
        $condition = $data['id'] > 0 ? ' AND c_hocki.id != '.$data['id'].'' : '';
        $name_exist =  $sql_model->queryWithResultSet('
            SELECT c_hocki.id  
            FROM c_hocki   
            WHERE UPPER(c_hocki.name) = UPPER("'.$data['name'].'") AND c_hocki.c_namhoc_code = "'.$data['c_namhoc_code'].'" '. $condition
        );
    ...
    function deleteFormData(){
        ...
        $result = Utility::processedDeleteData('c_hocki', $data);
        if($result['status'] == "OK"){
            $response['status'] = 'OK';
        } 
        return $response;
