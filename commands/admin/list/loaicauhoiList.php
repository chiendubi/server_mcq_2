...
class loaicauhoiList{
    ...
    function getDataList(){
        ...
        foreach($result['data'] as $rs){
            $adata[] = array(
                'DT_RowId' => 'row_'.$rs['C_loaicauhoi']['id'],
    ...
    function saveFormData(){
        ...
        if($data['default']['id'] > 0){
            ...
            $data['default']['code'] = Utility::processedCheckField('c_loaicauhoi', 'code', $data['default']['name'], true);
        }
        ...
        if($name_exist == 1){
            ...
            $result = Utility::processedSaveData('c_loaicauhoi', $data['default'], $skip, $json);
            if($result['status'] == "OK"){
                $response['status'] = 'OK';
    ...
    function deleteFormData(){
        ...
        $result = Utility::processedDeleteData('c_loaicauhoi', $data);
        if($result['status'] == "OK"){
            $response['status'] = 'OK';
        } 
        return $response;
