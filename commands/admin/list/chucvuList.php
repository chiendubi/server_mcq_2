...
class chucvuList{
    ...
    function getDataList(){
        ...
        foreach($result['data'] as $rs){
            $adata[] = array(
                'DT_RowId' => 'row_'.$rs['C_chucvu']['id'],
                'c_chucvu' => $rs['C_chucvu']
            );
        }
        $response = array(
    ...
    function saveFormData(){
        ...
        if($data['default']['id'] > 0){
            ...
            $data['default']['code'] = Utility::processedCheckField('c_chucvu', 'code', $data['default']['name'], true);
        }
        ...
        if($name_exist == 1){
            ...
            $result = Utility::processedSaveData('c_chucvu', $data['default'], $skip, $json);
            if($result['status'] == "OK"){
                $response['status'] = 'OK';
    ...
    function deleteFormData(){
        ...
        $result = Utility::processedDeleteData('c_chucvu', $data);
        if($result['status'] == "OK"){
            $response['status'] = 'OK';
        } 
        return $response;
