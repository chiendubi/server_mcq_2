...
class loaibaikiemtraList{
    ...
    function getDataList(){
        ...
        $table = 'c_loaibaikiemtra';
        $query = '
            SELECT 
                c_loaibaikiemtra.* 
            FROM c_loaibaikiemtra 
            WHERE 1 
        ';
        $condition = '';
        ...
        foreach($result['data'] as $rs){
            $adata[] = array(
                'DT_RowId' => 'row_'.$rs['C_loaibaikiemtra']['id'],
                'c_loaibaikiemtra' => $rs['C_loaibaikiemtra']
            );
        }
        $response = array(
    ...
    function saveFormData(){
        ...
        if($data['default']['id'] > 0){
            ...
            $data['default']['code'] = Utility::processedCheckField('c_loaibaikiemtra', 'code', $data['default']['name'], true);
        }
        ...
        if($name_exist == 1){
            ...
            $result = Utility::processedSaveData('c_loaibaikiemtra', $data['default'], $skip, $json);
            if($result['status'] == "OK"){
                $response['status'] = 'OK';
    ...
    function deleteFormData(){
        ...
        $result = Utility::processedDeleteData('c_loaibaikiemtra', $data);
        if($result['status'] == "OK"){
            $response['status'] = 'OK';
        } 
        return $response;
